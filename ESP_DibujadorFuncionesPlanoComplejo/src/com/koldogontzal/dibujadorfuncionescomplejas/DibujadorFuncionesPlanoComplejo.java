...
public class DibujadorFuncionesPlanoComplejo extends JPanel {
	...
	public DibujadorFuncionesPlanoComplejo(NumeroComplejo infIzq, NumeroComplejo supDrch, boolean ejes, boolean leyenda) {
	...
	public void paint(Graphics g) {
		...
		// TODO: Pensar si poner aquí esta función o delegarlar al objeto
		// PlanoComplejoDibujable
	...
	private void dibujaEje(Graphics g, NumeroComplejo zini, NumeroComplejo zfin, boolean esEjePrincipal) {
		...
		for (int j = valorInicioLinea; j<= valorFinalLinea; j++) {
			...
			NumeroComplejo zaux = this.convEspacial.convertir(x, y);
			NumeroComplejo resaux = funcionComplejaParaDibujar(zaux);
			ColorMod colaux = this.convColor.convertir(resaux);
			ColorMod col = this.convColor.convertir(funcionComplejaParaDibujar(this.convEspacial.convertir(x, y)));
	...
	private void pintaFuncionCompleja(Graphics g) {
		...
		for (int x = 0; x < ancho; x = x + paso) {
			for (int y = 0; y < alto; y = y + paso) {
				NumeroComplejo fz = funcionComplejaParaDibujar(this.convEspacial.convertir(x, y));
	...
	*/
		...
		//System.out.println(FuncionesComplejas.potencia(FuncionesComplejas.const_i, FuncionesComplejas.const_i));
